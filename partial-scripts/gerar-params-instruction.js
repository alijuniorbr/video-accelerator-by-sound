...
* **\`-p MS\`, \`--speech-padding MS\`**
    * **Descrição:** (Etapa 1: Segmentação de Áudio) Define a quantidade de preenchimento (ou "folga"), em milissegundos (MS), a ser adicionada *antes* do início detectado de um segmento de fala. Isso ajuda a garantir que o comecinho da voz não seja cortado. O segmento de silêncio anterior será correspondentemente encurtado.
    * **Tipo:** Inteiro
...
const outputPath = path.join(__dirname, "pv_process_params.md");

fs.writeFile(outputPath, markdownContent.trim(), (err) => {
  if (err) {
    console.error("Erro ao criar o arquivo Markdown:", err);
    return;
  }
  console.log(
    `Arquivo 'pv_process_params.md' criado com sucesso em: ${outputPath}`
  );
});
